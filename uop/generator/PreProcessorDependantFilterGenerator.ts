import PreProcessorDependantQueryRecordPropertyFilter
  from '../../queryAnalysis/PreProcessorDependantQueryRecordPropertyFilter.js'
 ...
 * a set of patterns dependant on record property pre-processors and determining whether the record matches any of
 * them.
 */
export default interface PreProcessorDependantFilterGenerator {
  /**
   * Generates a micro-code fragment that will test a provided record against the provided filter alternatives until it
   * finds a match while applying associated property pre-processors on the tested record. The code fragment will
   * perform a jump to an operation right after its last operation if a match is found so that it can be joined with
   * other code fragments performing other additional operations.
   *
   ...
   * @template I The type of the record with the pre-processors applied to test by the generated code.
   ...
   *        exclusion and exit operations are always generated for consistency). The elements of the top-level array
   *        are the individual alternative matching patterns, of which at least one must be satisfied, otherwise the
   *        record is excluded from query results.
   ...
   */
  <R, I>(filterAlternatives: readonly (readonly PreProcessorDependantQueryRecordPropertyFilter<R, I>[])[]): RecordOp[]
}
