import PreProcessorDependantQueryUniqueValueConstraint
  from '../../queryAnalysis/PreProcessorDependantQueryUniqueValueConstraint.js'
 ...
 * properties dependant on record pre-processors for having unique values.
 */
export default interface PreProcessorDependantUniqueValueCheckGenerator {
  /**
   * Generates a micro-code fragment that will test the pre-processor-dependant properties specified by the provided
   * constraints on a provided record for unique values while applying associated property pre-processors on the tested
   * record. The code fragment will perform a jump to an operation right after its last operation if all tested
   * properties have unique values so that it can be joined with other code fragments performing other additional
   * operations.
   *
   ...
   */
  <R, I>(constraints: readonly PreProcessorDependantQueryUniqueValueConstraint<R, I>[]): RecordOp[]
}
